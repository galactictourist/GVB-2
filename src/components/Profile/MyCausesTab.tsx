...
const MyCausesTab = () => {
  ...
  return (
    <div className="relative flex pt-4 pb-8">
      {causes &&
        causes.map((cause) => (
          <Link key={cause.id} href={`/cause/${cause.topic.parentId}`}>
            <div
              className={`mx-3 flex cursor-pointer items-center rounded-3xl py-3 px-6 text-white transition duration-700 hover:opacity-50 ${getCauseBgColor(
                cause.cause
              )}`}
              style={{
